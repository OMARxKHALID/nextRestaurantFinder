...
function BusinessItem({ business, showDir = false }) {
  ...
  useEffect(() => {
    if (userLocation && business?.geometry?.location) {
      calculateDistance(
  ...
  const onDirectionClick = () => {
    if (!userLocation || !business?.geometry?.location) return;
    const { lat, lng } = userLocation;
    const { lat: destLat, lng: destLng } = business.geometry.location;
    window.open(
